...
const ELEMENT_ID = 'test-html-display';
...
test.describe('Content rendering', () => {
  ...
  test('Renders inline code mark', async ({ page }) => {
    await elementClient.update(ELEMENT_ID, {
      content: '<p>run <code>npm test</code></p>',
    });
    await page.reload({ waitUntil: 'networkidle' });
    const display = new Display(page);
    await expect(display.content.locator('p > code')).toHaveText('npm test');
  });

  test('Renders images with src and alt', async ({ page }) => {
    await elementClient.update(ELEMENT_ID, {
      content: `<p><img src="${IMAGE_URL}" alt="Sunset" title="Sunset"></p>`,
    });
    await page.reload({ waitUntil: 'networkidle' });
    const display = new Display(page);
    const img = display.content.locator('img');
    await expect(img).toHaveAttribute('src', IMAGE_URL);
    await expect(img).toHaveAttribute('alt', 'Sunset');
  });

  test('Renders tooltip spans preserving class and data attribute', async ({
    page,
  }) => {
    await elementClient.update(ELEMENT_ID, {
      content:
        '<p><span class="has-tooltip" data-tooltip="explanation">term</span></p>',
    });
    await page.reload({ waitUntil: 'networkidle' });
    const display = new Display(page);
    const span = display.content.locator('span.has-tooltip');
    await expect(span).toHaveText('term');
    await expect(span).toHaveAttribute('data-tooltip', 'explanation');
  });
  ...
  test('Preserves font family inline style', async ({ page }) => {
    await elementClient.update(ELEMENT_ID, {
      content: '<p><span style="font-family: Georgia">serif</span></p>',
    });
    await page.reload({ waitUntil: 'networkidle' });
    const display = new Display(page);
    await expect(display.content.locator('span')).toHaveAttribute(
      'style',
      /font-family:\s*Georgia/,
    );
  });
